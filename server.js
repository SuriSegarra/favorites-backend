//loading enviroment variables form the .env file
...
app.post('/api/me/favorites', async(req, res) => {
    try {
        ...
        const newFavorites = await client.query(`
            INSERT INTO favorites (pokemon type_1, attack, user_id)
            values ($1, $2, $3, $4)
            returning *
        `, [
            pokemon,
...
app.get('/api/swapi', async(req, res) => {
    try {
        const data = await request.get(`https://alchemy-pokedex.herokuapp.com/api/pokedex/?search=${req.query.search}`);
